...
// MARK: - Set-button Live Activity Intent
//
// MUST be in a file that's compiled into BOTH the App target and the
// WorkoutWidget target. `LiveActivityIntent.perform()` runs in the
// *host app's* process (not the widget extension) — so the App
// binary needs the type's metadata at runtime, otherwise iOS can
// not resolve the intent at tap time and the buttons silently
// fire nothing. This file is registered for both targets in
// project.pbxproj.

/// Tapping a numbered set button on the Lock Screen / Dynamic Island
/// fires this intent. It is delivered to the main app's process
/// (running in the background if needed), so we can touch shared
/// `WorkoutGroupStore` state and the `Activity<>` instance directly.
///
...
///        drains it on next foreground / next launch and writes the
///        row to Supabase).
...
///      new state without round-tripping through the main app UI.
///
/// Explicit `description` + no-arg `init()` + parameterised init
/// follow Apple's WWDC23 sample for `LiveActivityIntent` — the
/// AppIntents metadata extractor's discovery is sensitive to these
/// being present. `isDiscoverable = false` hides the intent from
/// the Shortcuts app and also helps the extractor index it reliably.
struct ToggleSetIntent: LiveActivityIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        // Toggle the per-set completion bit. Explicit assignment rather
        // than `.toggle()` so the mutation is unambiguous to the compiler
        // (`.toggle()` on a Bool inside an array subscript has
        // occasionally shown up as a no-op in App Intent contexts).
